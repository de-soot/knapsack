# Knapsack
Implementation of the naive recursive depth-first search algorithm for the subset sum problem in C.
...
## How It Works
1. The program checks if the set is sorted, and sorts it from smallest to largest (using iterative selection sort) if it is not. This is to ensure the valid subset that is found first will have the lowest possbile amount of elements out of all valid subsets.
