...
2. The recursive function is called and combs through all the possible subsets recursively, choosing either to take or skip each element in the set, until it finds a valid subset. Each function call creates 2 new branches like a binary tree, and all the elements have to summed in the case that the subset is the same as the original set, so the algorithm has time complexity of `O(2^n * n)` (where `n` is the number of elements in the original set).
