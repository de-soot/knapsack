...
## How It Works
1. The program first sorts the set in ascending (non-negative target) and descending order (negative target) using iterative selection sort. This is to ensure the valid subset that is found first will have the lowest possbile amount of elements out of all valid subsets.
...
4. If the target is non-negative, the subset will exclude all the elements first so the first subset found will have the least amount of elements. When backtracking, it will include the last element and branches down until it either finds a valid subset or it reaches the end of the set, in which it then backtracks again to take the next last element.
